...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="测风数据邮件日报监测工具")
...
def main() -> int:
    ...
    xlsx_path = generate_xlsx_report(report_dir, stat_date, daily_result, logger)
    html_path = generate_html_report(report_dir, stat_date, daily_result, logger)

    should_send = bool(config["report"].get("send_email", True)) and not args.no_send
    if should_send:
        send_report_email(config, stat_date, html_path, xlsx_path, logger)
    else:
